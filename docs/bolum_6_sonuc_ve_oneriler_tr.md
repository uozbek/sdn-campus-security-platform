...
Ana çalışma zamanı doğrulama deneylerinde normal/kontrol akışlarının korunabildiği, zararlı UDP akışlarının yüksek saldırı olasılığı ile tespit edilebildiği ve bu tespitlerin denetleyici taraflı önleme aksiyonlarına dönüştürülebildiği gözlenmiştir. Özellikle run_05_port_aware_repeat_validation koşusu, port ve protokol farkındalıklı doğrulama açısından ana deney koşusu olarak değerlendirilmiştir.

Bu koşuda denetleyici politika distribution, final model prediction distribution, protocol-aware final policy distribution, önleme aksiyonu summary ve akış düzeyinde model-controller comparison çıktıları üretilmiştir. Deney sonucunda rate-limit, drop ve quarantine aksiyonlarının gözlenmesi, önerilen sistemin yalnızca pasif bir IDS değil, aktif bir IPS prototipi olarak da değerlendirilebileceğini göstermektedir.

## 6.4. Araştırma Sorularına Yanıtlar

Bu tez çalışmasının başlangıcında üç temel araştırma sorusu belirlenmiştir.

Birinci araştırma sorusu, SDN denetleyicisinin çalışma zamanı akış istatistikleri ve PCAP tabanlı özellik çıkarımı ile makine öğrenmesi modelini birlikte kullanıp kullanamayacağıdır. Geliştirilen prototip, bu soruya olumlu yanıt vermektedir. Ryu denetleyicisi akış istatistikleri verilerini toplamış, PCAP tabanlı pipeline seçilmiş özellikleri çıkarmış ve FastAPI üzerinden çalışan model çıkarım servisiyle birlikte çalışmıştır.

İkinci araştırma sorusu, eğitilmiş makine öğrenmesi modelinin karma normal/zararlı trafik senaryosunda zararlı UDP davranışını ayırt edip edemeyeceğidir. Runtime prediction sonuçları, normal akışların NORMAL, zararlı UDP akışlarının ise SALDIRI olarak sınıflandırılabildiğini göstermiştir. Bu sonuç, modelin çalışma zamanı veriyle de anlamlı tahminler üretebildiğini göstermektedir.

Üçüncü araştırma sorusu, model çıktısının SDN denetleyicisi üzerinde rate-limit, drop ve quarantine gibi aktif önleme aksiyonlarına dönüştürülüp dönüştürülemeyeceğidir. run_05 deneyinde bu üç aksiyonun da gözlenmesi, model çıktısı ve denetleyici taraflı politika motoru birlikteliğinin aktif IPS davranışı üretebildiğini desteklemektedir.

## 6.5. Çalışmanın Akademik Katkıları

Bu tez çalışmasının akademik katkıları birkaç başlık altında özetlenebilir.

İlk katkı, DDoS tespitini yalnızca çevrimdışı sınıflandırma problemi olarak ele almaması, bunu SDN çalışma zamanı ortamına taşımasıdır. Bu yönüyle çalışma, çevrimdışı makine öğrenmesi modeli ile çalışma zamanı SDN önleme davranışı arasında bir köprü kurmaktadır.

İkinci katkı, port ve protokol farkındalıklı değerlendirme yaklaşımının deneysel sürece dahil edilmesidir. Aynı kaynak ve hedef IP çifti arasında normal TCP, normal UDP, zararlı UDP ve karantina sonrası gözlenen akışların bulunabileceği gösterilmiş; bu nedenle doğru değerlendirme için port ve protokol bilgisinin zorunlu olduğu ortaya konmuştur.
...
Dördüncü katkı, deneysel çıktı üretiminin sistematik hale getirilmesidir. Politika karar kayıtları, tahmin dosyaları, akış istatistikleri, önleme kayıtları, karantina kayıtları, rate-limit kayıtları, protokol farkındalıklı karşılaştırma raporları, ana doğrulama özet raporları, tablo ve şekil çıktıları tez yazımına doğrudan aktarılabilecek şekilde paketlenmiştir.
...
Ayrıca çalışma, deneysel süreçte karşılaşılabilecek pratik sorunları da ortaya koymuştur. PCAP boyutu, tcpdump filtreleme, port bilgisinin controller loglarına eklenmesi, FastAPI servisinin erişilebilirliği, özellik şeması uyumluluğu ve akış düzeyinde karşılaştırma gibi konular, çalışma zamanı IDS/IPS geliştirme sürecinde dikkat edilmesi gereken pratik başlıklardır.
...
İkinci sınırlılık, kullanılan modelin seçilmiş Top-20 özellik setiyle çalışmasıdır. Bu yaklaşım çalışma zamanı uygulanabilirlik açısından avantaj sağlasa da daha geniş veri kümeleri ve daha farklı saldırı türleriyle yeniden değerlendirilmelidir.

Üçüncü sınırlılık, deney tekrar sayısının henüz sınırlı olmasıdır. run_03 ve run_05 ana doğrulama sonuçları destekleyici bulgular sunmakla birlikte, daha yüksek istatistiksel güven için farklı trafik oranları, farklı saldırı tipleri ve daha fazla tekrar gereklidir.

Dördüncü sınırlılık, denetleyici ek yükü ve ölçeklenebilirlik analizlerinin henüz tam kapsamlı yapılmamış olmasıdır. Denetleyici CPU/RAM kullanımı, akış istatistiği sorgulama yükü, çıkarım gecikmesi, API gidiş-dönüş süresi ve OpenFlow kural sayısı ileride daha ayrıntılı ölçülmelidir.
...
Üçüncü olarak, denetleyici ek yükü ayrıntılı biçimde analiz edilmelidir. Bu analizde akış istatistikleri toplama sıklığı, model çıkarım gecikmesi, API gidiş-dönüş süresi, OpenFlow kural kurulum gecikmesi ve denetleyici üzerindeki işlem yükü birlikte değerlendirilmelidir.
...
Bu tez çalışması, SDN tabanlı kampüs ağı benzeri bir ortamda makine öğrenmesi destekli DDoS tespiti ve aktif önleme mekanizmalarının birlikte çalışabileceğini destekleyen deneysel bulgular sunmuştur. Geliştirilen prototip, Final XGBoost Top-20 modeli, çalışma zamanı PCAP özellik çıkarımı, FastAPI çıkarım servisi, Ryu denetleyicisi ve OpenFlow tabanlı önleme mekanizmalarını aynı deneysel zincir içinde birleştirmiştir.

Ana deneyler sonucunda normal trafiğin korunduğu, zararlı UDP trafiğinin tespit edildiği ve rate-limit, drop ve quarantine aksiyonlarının uygulanabildiği görülmüştür. Bu sonuç, önerilen yaklaşımın yalnızca çevrimdışı bir saldırı tespit modeli değil, çalışma zamanı SDN tabanlı aktif IDS/IPS prototipi olarak değerlendirilebileceğini göstermektedir.

Sonuç olarak bu çalışma, SDN tabanlı ağ güvenliği alanında makine öğrenmesi destekli tespit ile programlanabilir ağ önleme mekanizmaları arasında uygulanabilir bir bütünleşme yaklaşımı sunmaktadır. Daha geniş veri kümeleri, daha fazla deney tekrarı, denetleyici ek yükü analizi ve gerçek testbed çalışmalarıyla bu yaklaşımın daha da güçlendirilmesi mümkündür.

## Tezin Akademik Katkısının Sonuç Açısından Değerlendirilmesi

Bu tez çalışması, yazılım tanımlı kampüs ağlarında DDoS saldırılarının makine öğrenmesi destekli olarak tespit edilmesi ve tespit sonucunun SDN denetleyicisi üzerinden aktif önleme aksiyonlarına dönüştürülmesi problemini ele almıştır. Elde edilen sonuçlar, önerilen mimarinin yalnızca çevrimdışı sınıflandırma başarımı üretmediğini; aynı zamanda çalışma zamanı trafik gözlemi, model tahmini, politika kararı ve OpenFlow tabanlı önleme adımlarını birlikte çalıştırabildiğini göstermektedir.

Çalışmanın akademik katkısı dört noktada özetlenebilir. Birincisi, makine öğrenmesi modelinin SDN çalışma zamanı ortamına entegre edilmesi ve model çıktısının denetleyici tarafındaki karar mekanizmasına bağlanmasıdır. İkincisi, port ve protokol farkındalıklı doğrulama yaklaşımıyla model çıktıları ile denetleyici taraflı politika kararları arasında daha izlenebilir bir eşleştirme kurulmasıdır. Üçüncüsü, IDS işlevinin drop, rate-limit ve quarantine_candidate gibi IPS aksiyonlarıyla aynı prototip içinde değerlendirilmesidir. Dördüncüsü ise deneysel çıktıların tablo, şekil, kalite kontrol raporu ve deneysel çıktı dosyalarıyla yeniden üretilebilir bir tez raporlama zincirine dönüştürülmesidir.

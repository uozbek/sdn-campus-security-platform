...
## 4.1. Yöntem Bölümüne Genel Bakış
...
- Denetleyici akış istatistikleri bilgilerini düzenli olarak toplar.
...
## 4.3. Geliştirilen SDN Güvenlik Platformunun Genel Mimarisi
...
## 4.5. Ryu Tabanlı SDN Denetleyicisi
...
- Paket oranı ve byte oranı gibi çalışma zamanı metriklerin hesaplanması
...
Bu çalışmada aktif çalışma zamanı model olarak Final XGBoost Top-20 modeli kullanılmıştır. Model, CIC-DDoS2019 veri kümesiyle uyumlu özellik temsili üzerinde eğitilmiştir. Özellik seçimi sonucunda çalışma zamanı uygulanabilirliğini artırmak amacıyla 20 özelliklik daha hafif bir özellik alt kümesi tercih edilmiştir.

Bu yaklaşımın iki temel nedeni vardır. Birincisi, SDN denetleyicisiyle entegre edilecek bir modelin düşük gecikme ile çalışması gerekmektedir. Çok yüksek boyutlu özellik vektörleri çalışma zamanı çıkarım süresini ve entegrasyon karmaşıklığını artırabilir. İkincisi, seçilmiş özelliklerle çalışan daha hafif bir model, deneysel prototip aşamasında daha kolay izlenebilir ve açıklanabilir sonuçlar üretmektedir.

Modelin çalışma zamanı pipeline’ında kullanılan genel akış şu şekildedir:

- PCAP dosyasından akış düzeyinde kayıtlar çıkarılır.
...
- Model `NORMAL` veya `SALDIRI` tahmini üretir.
...
**Şekil 4.5. Önerilen SDN tabanlı IDS/IPS mimarisinin bileşenleri**

**Şekil 4.6. Model geliştirme ve nihai model seçimi iş akışı**

**Şekil 4.7. Çalışma zamanı IDS/IPS karar döngüsü**
...
Bu amaçla geliştirilen çalışma zamanı iş akışı aşağıdaki alt adımlardan oluşmaktadır:
...
## 4.9. Hibrit Politika Karar Mekanizması
...
$$
p_t = P(y_t = \mathrm{saldırı} \mid \mathbf{x}_t)
$$
...
## 4.11. Loglama ve Deneysel İzlenebilirlik
...
## 4.12. Model-Controller Karşılaştırma Yöntemi
...
Exact match, model çıktısı ve denetleyici loglarının aynı akış anahtarı üzerinde buluşup buluşmadığını gösterir. Security-compatible match ise aksiyon adları birebir aynı olmasa bile güvenlik açısından uyumlu bir davranışın oluşup oluşmadığını değerlendirir.
...
## 4.13. Deneylerin Sınıflandırılması
...
## 4.14. Tezde Kullanılan Ana Deney Paketi

Ana tez raporlaması için kullanılan çıktı dosyası paketi aşağıdaki dizinde tutulmaktadır:
...
## 4.15. Yöntemin Genel Değerlendirmesi
...
## 4.16. Önerilen SDN Tabanlı IDS/IPS Prototipinin Çalışma Zamanı Doğrulaması

### 4.16.1. Çalışma Zamanı Doğrulama Aşamasına Genel Bakış
...
### 4.16.4. Tekrarlı Çalışma Zamanı Doğrulama Sonuçları



**Tablo 4.1. Canonical tekrarlı çalışma zamanı doğrulama sonuçları**  
Kaynak: `tables/table_canonical_runtime_validation_summary.csv`

Tablo 4.1, tez raporlamasında kullanılmak üzere seçilen canonical çalışma zamanı doğrulama koşularını özetlemektedir. Bu kapsamda `run_03_aligned_clean` ve `run_05_port_aware_repeat_validation` koşuları canonical doğrulama koşuları olarak değerlendirilmiştir.
...
**Tablo 4.2. Denetleyici taraflı IDS/IPS politika aksiyon dağılımı**  
Kaynak: `tables/table_controller_action_distribution.csv`



**Şekil 4.1. Denetleyici taraflı politika aksiyon dağılımı**  
...
### 4.16.6. Final XGBoost Top-20 Modelinin Runtime Tahmin Dağılımı



**Tablo 4.3. Final XGBoost Top-20 modelinin çalışma zamanı tahmin dağılımı**  
Kaynak: `tables/table_final_top20_prediction_distribution.csv`



**Şekil 4.2. Çalışma zamanı NORMAL/SALDIRI tahmin dağılımı**  
...
### 4.16.7. Protocol-Aware Final Policy Yorumu



**Tablo 4.4. Protokol farkındalıklı nihai politika dağılımı**  
Kaynak: `tables/table_protocol_aware_final_policy_distribution.csv`



**Şekil 4.3. Protokol farkındalıklı nihai politika aksiyon dağılımı**  
...
### 4.16.8. SDN Denetleyicisi Önleme Aksiyonları



**Tablo 4.5. SDN denetleyicisi önleme aksiyon özeti**  
Kaynak: `tables/table_enforcement_action_summary.csv`



**Şekil 4.4. SDN denetleyicisi önleme aksiyon özeti**  
...
### 4.16.9. Flow-Level Model ve Denetleyici Karşılaştırması



**Tablo 4.6. Akış düzeyinde model-denetleyici karşılaştırması**  
Kaynak: `tables/table_flow_level_model_controller_comparison.csv`

Flow-level karşılaştırma bu doğrulama aşamasının en kritik çıktılarından biridir. Bu tablo, çalışma zamanı model çıktıları ile denetleyici taraflı politika kayıtlarını kaynak IP, hedef IP, kaynak port, hedef port ve taşıma protokolü düzeyinde karşılaştırmaktadır.

Deneydeki temel zararlı akış, `10.10.60.12` adresinden `10.10.40.14` adresine giden UDP akışıdır. Bu akış çalışma zamanı model tarafından `SALDIRI` olarak sınıflandırılmış ve protocol-aware final policy çıktısında `DROP` olarak yorumlanmıştır. Denetleyici tarafında ise aynı akış `quarantine_candidate` aksiyonu ile ilişkilendirilmiştir. Ayrıca aynı akış için drop, quarantine ve rate-limit kayıtlarının da eşleştiği görülmektedir.
...
### 4.16.10. Saldırı Olasılığı ve Final Policy Eşlemesi



**Tablo 4.7. Akış düzeyinde saldırı olasılığı ve protokol farkındalıklı nihai aksiyon özeti**  
Kaynak: `tables/table_protocol_aware_attack_probability_summary.csv`

Saldırı olasılığı özeti, Final XGBoost Top-20 modelinin her çalışma zamanı akış için ürettiği saldırı olasılıklarını göstermektedir. Zararsız akışlar düşük saldırı olasılıkları almış ve `ALLOW` ya da `ALLOW_CONTROL_FLOW` olarak yorumlanmıştır. Buna karşılık zararlı UDP akışı yüksek saldırı olasılığı almış ve `DROP` aksiyonuna eşlenmiştir.
...
4. Final XGBoost Top-20 modelinden çalışma zamanı tahmin alınması
...
### 4.16.12. Tez Açısından Katkı

Bu deneysel aşama teze üç temel katkı sağlamaktadır.

İlk olarak, çevrimdışı olarak eğitilmiş seçilmiş özellik tabanlı bir makine öğrenmesi modelinin çalışma zamanı SDN pipeline’ına entegre edilebildiği gösterilmiştir. Final XGBoost Top-20 modeli, PCAP üzerinden çıkarılan akış özellikleri üzerinde anlamlı normal/saldırı tahminleri üretmiştir.
...
Canonical port-aware çalışma zamanı doğrulaması deneyi, önerilen SDN tabanlı hibrit IDS/IPS prototipinin canlı bir SDN test ortamında çalışabildiğini göstermektedir. Sistem zararsız ve kontrol trafiğini korumuş, zararlı UDP davranışını yüksek saldırı olasılığı ile tespit etmiş ve denetleyici tarafında rate-limit, drop ve quarantine aksiyonları üretmiştir.
...
## 4.17. Bölüm 4 Genel Kapanış Değerlendirmesi

Bu bölümde geliştirilen SDN tabanlı makine öğrenmesi destekli IDS/IPS prototipinin hem yöntemsel tasarımı hem de çalışma zamanı doğrulama sonuçları sunulmuştur. İlk olarak sistem mimarisi, deney ortamı, Ryu tabanlı denetleyici, FastAPI üzerinden çalışan Final XGBoost Top-20 modeli, çalışma zamanı özellik çıkarımı pipeline ve protocol-aware final policy katmanı açıklanmıştır. Ardından bu bileşenlerin karma normal ve zararlı trafik senaryosu altında nasıl çalıştığı deneysel olarak incelenmiştir.

Elde edilen bulgular, önerilen sistemin normal ve kontrol trafiğini koruyabildiğini, zararlı UDP davranışını yüksek saldırı olasılığı ile tespit edebildiğini ve bu tespiti SDN denetleyicisi üzerinde rate-limit, drop ve quarantine gibi önleme aksiyonlarına dönüştürebildiğini göstermektedir. Özellikle port-aware ve protocol-aware karşılaştırma yaklaşımı, model çıktısı ile denetleyici davranışının daha doğru ilişkilendirilmesini sağlamıştır.

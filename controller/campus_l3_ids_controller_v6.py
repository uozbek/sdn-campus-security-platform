...
class CampusL3IdsControllerV6(app_manager.RyuApp):
    """
    Campus L3 IDS Controller V6.

    New in V6:
    - Policy Engine added on top of ML API recommendations.
    - ML recommended_action is not directly applied.
    - Final policy decision is logged to policy_decisions.csv.
    - Source IP risk counters are tracked for repeated attack-like behavior.
    - Repeated high-confidence attack decisions produce quarantine_candidate.

    This version still does not apply real mitigation.
    It only logs policy decisions for the next mitigation phase.
    """

    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(CampusL3IdsControllerV6, self).__init__(*args, **kwargs)
        ...
        self.logger.info("Campus L3 IDS Controller V6 started")
    ...
    def evaluate_policy(self, features, ml_result):
        ...
        current_risk_count = self.source_risk_counters.get(src_ip, 0)
        ...
        if confidence < POLICY_MONITOR_THRESHOLD:
            ...
            # Slowly reduce risk for benign/low-confidence flows.
            if current_risk_count > 0:
                self.source_risk_counters[src_ip] = current_risk_count - 1
        ...
        updated_risk_count = self.source_risk_counters.get(src_ip, 0)
        ...
        # Fail open if ML API is not healthy.
        ...
        return {
            ...
            "reason": reason,
            "source_risk_count": self.source_risk_counters.get(src_ip, 0),

"""
campus_l3_ids_controller_v11_runtime_metrics.py

Bu sürüm, campus_l3_ids_controller_v10_observe_only.py dosyasından türetilmiştir.

Amaç:
- L3 routing ve IDS/ML prediction davranışını korumak
- Observe-only modda normal trafik baseline testlerini bozmamak
- Controller runtime ölçümleri için flow rule installation timing logları üretmek
- OpenFlow FlowMod hazırlama/gönderme sürelerini logs/flow_rule_timing.csv dosyasına yazmak

Not:
- Bu sürümde mitigation uygulanmaz.
- Drop/rate-limit/quarantine aksiyonları policy olarak üretilebilir; fakat OpenFlow mitigation kuralı kurulmaz.
"""
...
FLOW_RULE_TIMING_LOG_FILE = os.path.join(LOG_DIR, "flow_rule_timing.csv")
...
class CampusL3IdsControllerV9(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._init_flow_rule_timing_log_file()
    ...
    def add_flow(
        ...
        try:
            self.append_flow_rule_timing_log(
            ...
            self.logger.warning("flow_rule_timing log failed: %s", e)
    ...
    def _init_flow_rule_timing_log_file(self):
        os.makedirs(LOG_DIR, exist_ok=True)

        if os.path.exists(FLOW_RULE_TIMING_LOG_FILE) and os.path.getsize(FLOW_RULE_TIMING_LOG_FILE) > 0:
            return

        with open(FLOW_RULE_TIMING_LOG_FILE, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                "timestamp",
                "datapath_id",
    ...
    def append_flow_rule_timing_log(
        self,
        datapath_id,
        src_ip,
        dst_ip,
        ...
        with open(FLOW_RULE_TIMING_LOG_FILE, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
    ...
    def install_drop_rules_for_flow(self, features, policy_result):
        if not ENABLE_MITIGATION:
            self.logger.info("[OBSERVE_ONLY] DROP mitigation skipped.")
            return
        src_ip = features.get("ipv4_src", "")
        dst_ip = features.get("ipv4_dst", "")
        ip_proto = features.get("ip_proto", None)
    ...
    def install_rate_limit_for_flow(self, features, policy_result):
        if not ENABLE_MITIGATION:
            self.logger.info("[OBSERVE_ONLY] RATE_LIMIT mitigation skipped.")
            return
        """
    ...
    def handle_ipv4(self, datapath, in_port, ip_pkt, msg):
        ...
        flow_install_ms = self.add_flow(
            ...
            idle_timeout=60,
            hard_timeout=0,
            rule_type="l3_forward",
            src_ip=src_ip,
            dst_ip=dst_ip,
            ip_proto=ip_proto,
            out_port=out_port,
            reason="packet_in_l3_forward"
        )
        ...
        self.logger.info(
            "IPv4 routed: dpid=%s src=%s dst=%s proto=%s out_port=%s dst_mac=%s flow_install_ms=%.6f",
            dpid,
            src_ip,
            dst_ip,
            ip_proto,
            out_port,
            dst_mac,
            flow_install_ms if flow_install_ms is not None else -1.0
        )

...
class CampusL3IdsControllerV5(app_manager.RyuApp):
    """
    Campus L3 IDS Controller V5.

    New in V5:
    - Delta-based packet_rate and byte_rate calculation
    - Protocol-aware flow rules using ip_proto
    - ML prediction only at source edge switch
    - Cleaner prediction logging with reduced duplicate decisions

    This version still does not apply mitigation.
    It only logs ML recommendations.
    """

    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(CampusL3IdsControllerV5, self).__init__(*args, **kwargs)
        ...
        self.logger.info("Campus L3 IDS Controller V5 started")
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        with open(FLOW_STATS_FILE, "a", newline="") as f:
            ...
            for stat in ev.msg.body:
                ...
                self.logger.info(
                    "ML prediction edge-only: dpid=%s src=%s dst=%s proto=%s "
                    "pps=%.2f bps=%.2f prediction=%s action=%s confidence=%s api=%s",
                    datapath_id,
                    ipv4_src,
                    ipv4_dst,
                    ip_proto,

"""
campus_l3_ids_controller_v13_decision_modes.py

Bu sürüm, campus_l3_ids_controller_v12_mitigation_metrics.py dosyasından türetilmiştir.

Amaç:
- heuristic_only, ml_only ve hybrid karar modlarını desteklemek
- Aynı Mininet trafik senaryosunu farklı karar stratejileriyle çalıştırmak
- Runtime IDS/IPS karşılaştırmalı deneyleri için log üretmek

Modlar:
- heuristic_only: yalnızca controller içi runtime heuristic kararları
- ml_only: yalnızca FastAPI /predict çıktısına göre karar
- hybrid: ML çıktısı + controller heuristic + repeated risk fusion
"""
import csv
import os
import json
...
# -------------------------------------------------------------------
# Controller decision mode configuration
# -------------------------------------------------------------------

CONTROLLER_DECISION_MODE = os.getenv(
...
MITIGATION_LATENCY_LOG_FILE = os.path.join(LOG_DIR, "mitigation_latency.csv")
...
class CampusL3IdsControllerV9(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if CONTROLLER_DECISION_MODE not in VALID_DECISION_MODES:
            self.logger.warning(
                "Invalid CONTROLLER_DECISION_MODE=%s, falling back to hybrid",
                CONTROLLER_DECISION_MODE,
            )

        self.decision_mode = (
            CONTROLLER_DECISION_MODE
            if CONTROLLER_DECISION_MODE in VALID_DECISION_MODES
            else "hybrid"
        )

        self.logger.info("Controller decision mode: %s", self.decision_mode)
        ...
        self._init_mitigation_latency_log_file()
    ...
    def _init_mitigation_latency_log_file(self):
        os.makedirs(LOG_DIR, exist_ok=True)

        with open(MITIGATION_LATENCY_LOG_FILE, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                "timestamp",
                "src_ip",
    ...
    def append_mitigation_latency_log(
        self,
        src_ip,
        dst_ip,
        ip_proto,
        ...
        with open(MITIGATION_LATENCY_LOG_FILE, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
    ...
    def local_runtime_heuristic_decision(self, features):
        """
        Controller-side runtime heuristic decision.

        FastAPI /predict sonucunu kullanmaz. Yalnızca controller'ın flow stats
        üzerinden hesapladığı packet_rate, byte_rate ve ip_proto değerleriyle
        karar üretir.
        """

        ip_proto = int(features.get("ip_proto") or 0)
        packet_rate = float(features.get("packet_rate", 0.0) or 0.0)
        byte_rate = float(features.get("byte_rate", 0.0) or 0.0)
        ...
        # UDP high-volume traffic: DDoS-like behavior in the Mininet runtime scenario.
        if ip_proto == 17:
            if packet_rate >= 2000 or byte_rate >= 3_000_000:
                return {
                    ...
                    "reason": "local_heuristic_very_high_udp_rate",
                }
            ...
            if packet_rate >= 800 or byte_rate >= 1_000_000:
                return {
                    ...
                    "reason": "local_heuristic_high_udp_rate",
                }
            ...
            if packet_rate >= 300 or byte_rate >= 500_000:
                return {
                    ...
                    "reason": "local_heuristic_moderate_udp_rate",
                }

        # TCP için daha temkinli davranıyoruz; normal iperf TCP testleri drop edilmemeli.
        if ip_proto == 6:
            if packet_rate >= 5000 or byte_rate >= 20_000_000:
                return {
                    ...
                    "confidence": 0.80,
                    "recommended_action": "monitor",
                    "reason": "local_heuristic_high_tcp_rate_monitor_only",
                }
        ...
        if ip_proto == 1:
            if packet_rate >= 1000:
                return {
                    ...
                    "confidence": 0.90,
                    "recommended_action": "rate_limit",
                    "reason": "local_heuristic_high_icmp_rate",
                }
        ...
        return {
            ...
            "confidence": 0.10,
            "recommended_action": "allow",
            "reason": "local_heuristic_below_threshold",
        }

    def build_mode_aware_ml_result(self, features, ml_result):
        """
        Controller decision mode'a göre evaluate_policy içine gönderilecek sonucu üretir.

        heuristic_only:
            FastAPI sonucu yok sayılır; controller-local heuristic kullanılır.

        ml_only:
            FastAPI /predict çıktısı aynen kullanılır.

        hybrid:
            FastAPI /predict çıktısı ile controller-local heuristic birlikte değerlendirilir.
        """

        local_result = self.local_runtime_heuristic_decision(features)

        if self.decision_mode == "heuristic_only":
            return {
                "prediction": local_result["prediction"],
                "confidence": local_result["confidence"],
                "recommended_action": local_result["recommended_action"],
                "model_id": "H001",
                "model_name": "controller_local_heuristic",
                "inference_latency_ms": 0.0,
                "controller_ml_roundtrip_ms": 0.0,
                "api_status": "ok",
                "decision_mode": "heuristic_only",
                "mode_reason": local_result["reason"],
            }

        if self.decision_mode == "ml_only":
            result = dict(ml_result)
            result["decision_mode"] = "ml_only"
            result["mode_reason"] = "fastapi_predict_only"
            return result

        # hybrid mode
        ml_prediction = str(ml_result.get("prediction", "unknown")).strip().lower()
        ml_confidence = float(ml_result.get("confidence", 0.0) or 0.0)

        local_prediction = str(local_result["prediction"]).strip().lower()
        local_action = str(local_result["recommended_action"]).strip().lower()
        local_confidence = float(local_result["confidence"])

        # ML ve local heuristic ikisi de benign ise güvenli allow.
        if ml_prediction in {"benign", "normal", "allow"} and local_prediction in {"benign", "normal", "allow"}:
            return {
                "prediction": "benign",
                "confidence": min(ml_confidence, local_confidence),
                "recommended_action": "allow",
                "model_id": ml_result.get("model_id", ""),
                "model_name": "hybrid_fusion",
                "inference_latency_ms": ml_result.get("inference_latency_ms", 0.0),
                "controller_ml_roundtrip_ms": ml_result.get("controller_ml_roundtrip_ms", 0.0),
                "api_status": ml_result.get("api_status", "unknown"),
                "decision_mode": "hybrid",
                "mode_reason": "ml_and_local_benign",
            }

        # Local heuristic yüksek UDP saldırı görüyorsa IPS tarafı bunu dikkate alır.
        if local_prediction == "attack":
            return {
                "prediction": "attack",
                "confidence": max(ml_confidence, local_confidence),
                ...
                "api_status": ml_result.get("api_status", "unknown"),
                "decision_mode": "hybrid",
                "mode_reason": "local_attack_override",
            }

        # ML attack diyorsa ama local heuristic bunu doğrulamıyorsa temkinli monitor.
        if ml_prediction in {"attack", "ddos", "ddos_suspected", "suspicious"}:
            return {
                "prediction": "suspicious",
                "confidence": max(0.75, min(ml_confidence, 0.85)),
                "recommended_action": "monitor",
                "model_id": ml_result.get("model_id", ""),
                "model_name": "hybrid_fusion",
                "inference_latency_ms": ml_result.get("inference_latency_ms", 0.0),
                ...
                "api_status": ml_result.get("api_status", "unknown"),
                "decision_mode": "hybrid",
                "mode_reason": "ml_attack_local_not_confirmed",
            }
    ...
    def evaluate_policy(self, features, ml_result):
        ...
        # Flow artık aktif değilse veya sadece eski flow stat kaydı dönüyorsa,
        # risk sayacını artırma ve mitigation üretme.
        if packet_rate <= 0.0 and byte_rate <= 0.0:
            return {
                "final_action": "allow",
                "reason": "zero_rate_flow_stats_ignored",
                "source_risk_count": self.source_risk_counters.get(
                    (
                        src_ip,
                        dst_ip,
                ...
                "ml_recommended_action": ml_result.get("recommended_action", "allow"),
                "ml_confidence": float(ml_result.get("confidence", 0.0) or 0.0)
            }
        ...
        ml_action_normalized = str(ml_action).strip().lower()

        # Safety guard:
        # Eğer model benign diyorsa, recommended_action=drop gelse bile
        # mitigation uygulanmaz. Bu durum runtime /predict endpoint'inde
        # prediction-action çelişkisi oluştuğunda normal trafiği korur.
        if prediction_normalized in {"benign", "normal", "allow"}:
            return {
                "final_action": "allow",
                "reason": "ml_prediction_benign_safety_override",
                "source_risk_count": self.source_risk_counters.get(
                    (
                        src_ip,
                        dst_ip,
        ...
        # Fail-open policy:
        # - In ml_only mode, if the ML API is not healthy, do not enforce mitigation.
        # - In hybrid / heuristic_only modes, controller-local heuristic decisions
        #   are allowed to enforce mitigation even when the ML API is unreachable.
        #   This preserves the intended hybrid IDS/IPS behavior.
        api_status = str(ml_result.get("api_status", "")).strip().lower()
        decision_mode = str(ml_result.get("decision_mode", self.decision_mode)).strip().lower()
        mode_reason = str(ml_result.get("mode_reason", "")).strip().lower()

        local_override_active = mode_reason in {
            "local_attack_override",
            "local_heuristic_high_udp_rate",
            "local_heuristic_very_high_udp_rate",
            "local_heuristic_high_icmp_rate",
        }

        if api_status != "ok" and decision_mode == "ml_only":
            final_action = "allow"
            reason = "ml_api_not_ok_fail_open"

        elif api_status != "ok" and decision_mode == "hybrid" and not local_override_active:
            final_action = "allow"
            reason = "ml_api_not_ok_fail_open_no_local_override"
    ...
    def append_policy_decision_log(self, timestamp, features, ml_result, policy_result):
        with open(POLICY_DECISIONS_FILE, "a", newline="") as f:
            ...
            writer.writerow([
                ...
                ml_result.get("decision_mode", self.decision_mode),
                ml_result.get("mode_reason", "")
            ])
    ...
    def install_drop_rules_for_flow(self, features, policy_result):
        ...
        for datapath_id, datapath in list(self.datapaths.items()):
            ...
            latency_ms = (time.perf_counter() - mitigation_start) * 1000.0
    ...
    def install_rate_limit_for_flow(self, features, policy_result):
        ...
        self.append_mitigation_latency_log(
            src_ip=src_ip,
            dst_ip=dst_ip,
            ip_proto=ip_proto if ip_proto is not None else "",
            policy_final_action=final_action,
    ...
    def extract_transport_ports_from_match(self, match):
        """
        Extract TCP/UDP source and destination ports from an OpenFlow match.

        The controller currently makes IDS/IPS decisions from flow statistics.
        Depending on how the flow was installed, transport-layer ports may or
        may not be present in stat.match. If ports are unavailable, return 0
        values so the log schema remains stable.
        """
        src_port = 0
        dst_port = 0
        ...
        try:
            # Ryu OFPMatch supports dict-like access for matched fields.
            if "tcp_src" in match:
                src_port = int(match.get("tcp_src") or 0)
            if "tcp_dst" in match:
                dst_port = int(match.get("tcp_dst") or 0)

            if src_port == 0 and "udp_src" in match:
                src_port = int(match.get("udp_src") or 0)
            if dst_port == 0 and "udp_dst" in match:
                dst_port = int(match.get("udp_dst") or 0)
    ...
    def update_transport_port_cache(self, src_ip, dst_ip, ip_proto, src_port, dst_port):
        """
        Store observed transport-layer ports from PacketIn events.

        Keyed by (src_ip, dst_ip, ip_proto). FlowStats entries may not contain
        tcp_src/tcp_dst or udp_src/udp_dst, so this cache fills that gap.
        """
        if not src_ip or not dst_ip or ip_proto is None:
            return

        try:
            key = (str(src_ip), str(dst_ip), int(ip_proto))
            self.transport_port_cache[key] = {
            ...
            self.logger.info(
                "Port cache update: src=%s:%s dst=%s:%s proto=%s cache_size=%s",
                src_ip,
                src_port,
                dst_ip,
                dst_port,
                ip_proto,
                len(self.transport_port_cache)
            )
        except Exception:
            return

    def lookup_transport_ports(self, src_ip, dst_ip, ip_proto):
        """
        Look up transport ports observed from PacketIn.

        First tries the same direction. If not found, tries reverse direction
        and swaps source/destination ports.
        """
        try:
            key = (str(src_ip), str(dst_ip), int(ip_proto))
            item = self.transport_port_cache.get(key)
            if item:
                return int(item.get("src_port", 0)), int(item.get("dst_port", 0))

            reverse_key = (str(dst_ip), str(src_ip), int(ip_proto))
            item = self.transport_port_cache.get(reverse_key)
            if item:
                return int(item.get("dst_port", 0)), int(item.get("src_port", 0))
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        with open(FLOW_STATS_FILE, "a", newline="") as f:
            ...
            for stat in ev.msg.body:
                ...
                src_port, dst_port = self.extract_transport_ports_from_match(match)

                if src_port == 0 and dst_port == 0:
                    src_port, dst_port = self.lookup_transport_ports(
                        ipv4_src,
                        ipv4_dst,
                        ip_proto
                    )
                ...
                if src_port or dst_port:
                    self.logger.info(
                        "FlowStats port resolved: src=%s:%s dst=%s:%s proto=%s",
                        ipv4_src,
                        src_port,
                        ipv4_dst,
                        dst_port,
                ...
                raw_ml_result = self.ask_ml_api(features)
                ml_result = self.build_mode_aware_ml_result(features, raw_ml_result)
                ...
                self.logger.info(
                    "Policy decision: dpid=%s src=%s:%s dst=%s:%s proto=%s "
                    "pps=%.2f bps=%.2f ml_prediction=%s ml_action=%s "
                    "confidence=%s final_action=%s reason=%s risk_count=%s "
                    "mode=%s mode_reason=%s api=%s",
                    datapath_id,
                    ipv4_src,
                    src_port,
                    ipv4_dst,
    ...
    def handle_ipv4(self, datapath, in_port, ip_pkt, msg):
        ...
        try:
            ...
            if src_port or dst_port:
                self.update_transport_port_cache(
                    src_ip=src_ip,
                    dst_ip=dst_ip,
                    ip_proto=ip_proto,
                    src_port=src_port,

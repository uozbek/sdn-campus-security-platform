...
class CampusL3IdsControllerV4(app_manager.RyuApp):
    """
    Campus L3 IDS Controller V4.

    This controller is the first integrated IDS-ready controller version.
    ...
    - CSV logging for flow statistics
    - ML inference API integration
    - CSV logging for predictions

    Important:
    - This version does NOT apply mitigation.
    - It only logs the recommended action returned by the ML API.
    """

    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(CampusL3IdsControllerV4, self).__init__(*args, **kwargs)
        ...
        self.logger.info("Campus L3 IDS Controller V4 started")
    ...
    def ask_ml_api(self, features):
        """
        Send flow features to the ML inference API.

        This version intentionally uses Python's built-in http.client
        instead of requests, because Ryu/eventlet environments may cause
        SSL recursion problems when importing requests/urllib3.

        Fail-open behavior:
        If the ML API is unreachable or returns an error, the controller
        records the error but recommends 'allow'.
        """
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        with open(FLOW_STATS_FILE, "a", newline="") as f:
            ...
            for stat in ev.msg.body:
                ...
                # Only process IPv4 source/destination flows.
                ...
                self.logger.info(
                    "ML prediction: src=%s dst=%s prediction=%s action=%s confidence=%s api=%s",
                    ipv4_src,
                    ipv4_dst,
                    ml_result.get("prediction", ""),
                    ml_result.get("recommended_action", ""),
    ...
    def handle_ipv4(self, datapath, in_port, ip_pkt, msg):
        ...
        if dst_ip in GATEWAY_IPS:
            # This controller replies to ARP for gateway IPs,

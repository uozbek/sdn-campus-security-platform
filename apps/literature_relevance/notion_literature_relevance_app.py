...
def build_dedup_key(row: pd.Series) -> str:
    ...
    citation_key = normalize_text(row.get("Citation Key", ""))
    if citation_key:
        return f"citation_key::{citation_key}"

    title = normalize_title_for_dedup(row.get("Title", ""))
    ...
    name = normalize_title_for_dedup(row.get("Name", ""))
    if name and year:
        return f"name_year::{name}::{year}"

    return f"row::{row.name}"


def choose_best_duplicate_row(group: pd.DataFrame) -> pd.Series:
    def row_quality_score(row):
        score = 0

        if normalize_doi(row.get("DOI", "")):
            score += 100
        if safe_str(row.get("File Path", "")).strip():
            score += 80
        if safe_str(row.get("Abstract", "")).strip():
            score += 70
        if safe_str(row.get("Full Citation", "")).strip():
            score += 40
        if safe_str(row.get("ZoteroNotes", "")).strip():
            score += 30
        if safe_str(row.get("Other Notes", "")).strip():
            score += 20
        if safe_str(row.get("URL", "")).strip():
            score += 10
        ...
        score += sum(len(safe_str(row.get(c, ""))) for c in text_fields) / 1000
        return score

    scored = group.copy()
    scored["_dedup_quality_score"] = scored.apply(row_quality_score, axis=1)
    scored = scored.sort_values("_dedup_quality_score", ascending=False)
    return scored.iloc[0].drop(labels=["_dedup_quality_score"])


def deduplicate_dataframe(df: pd.DataFrame, enable_dedup: bool = True) -> tuple:
    if not enable_dedup:
        df = df.copy()
        df["Duplicate_Key"] = df.apply(build_dedup_key, axis=1)
        df["Duplicate_Count"] = 1
        df["Duplicate_Status"] = "dedup_disabled"
        return df, pd.DataFrame()

    work = df.copy()
    work["Duplicate_Key"] = work.apply(build_dedup_key, axis=1)
    work["Duplicate_Count"] = work.groupby("Duplicate_Key")["Duplicate_Key"].transform("count")

    selected_rows = []
    duplicate_report_rows = []

    for key, group in work.groupby("Duplicate_Key", sort=False):
        if len(group) == 1:
            row = group.iloc[0].copy()
            row["Duplicate_Status"] = "unique"
            selected_rows.append(row)
        else:
            best = choose_best_duplicate_row(group).copy()
            best["Duplicate_Key"] = key
            best["Duplicate_Count"] = len(group)
            best["Duplicate_Status"] = "kept_best_of_duplicates"
            selected_rows.append(best)

            duplicate_report_rows.append({
                "Duplicate_Key": key,
                "Duplicate_Count": len(group),
                "Kept_Title": safe_str(best.get("Title", "")),
                "All_Titles": " || ".join(group["Title"].fillna("").astype(str).tolist()) if "Title" in group.columns else "",
                "All_DOIs": " || ".join(group["DOI"].fillna("").astype(str).tolist()) if "DOI" in group.columns else "",
                "All_Citation_Keys": " || ".join(group["Citation Key"].fillna("").astype(str).tolist()) if "Citation Key" in group.columns else "",
                "All_Source_Files": " || ".join(group["Source_CSV_File"].fillna("").astype(str).tolist()) if "Source_CSV_File" in group.columns else "",
            })
...
def extract_summary_from_text(abstract_final: str, full_text: str, max_sentences: int = 4):
    ...
    return compact_text(" ".join(sentences[:max_sentences]), max_chars=1400)
...
def analyze_dataframe(
    ...
    df, duplicate_report = deduplicate_dataframe(
        df,
        enable_dedup=enable_dedup
    )

    deduped_count = len(df)
    removed_duplicate_count = original_count - deduped_count
    ...
    out_scope_count_before_filter = int((enriched["Thesis_Relevance_Label"] == "Out of scope").sum())

    if not include_out_of_scope:
        enriched = enriched[enriched["Thesis_Relevance_Label"] != "Out of scope"].copy()
    ...
    enriched = enriched[first_cols + analysis_cols + remaining]

    metadata = {
        "original_count": int(original_count),
        "deduped_count": int(deduped_count),
        "removed_duplicate_count": int(removed_duplicate_count),
        "final_output_count": int(len(enriched)),
        "out_of_scope_count_before_filter": int(out_scope_count_before_filter),
        "out_of_scope_removed": int(out_scope_count_before_filter if not include_out_of_scope else 0),
        "enable_dedup": bool(enable_dedup),
        "include_out_of_scope": bool(include_out_of_scope),
        "use_fulltext": bool(use_fulltext),
        "max_pdf_pages": int(max_pdf_pages),
        "abstract_update_mode": abstract_update_mode,
    }

    return enriched, duplicate_report, metadata


def write_markdown_report(df: pd.DataFrame, duplicate_report: pd.DataFrame, metadata: dict, out_md: Path):
    lines = []
    lines.append("# Notion/Notero Literature Relevance Report")
    lines.append("")
    lines.append("## 1. Özet")
    lines.append("")
    lines.append(f"- Girdi kayıt sayısı: `{metadata.get('original_count')}`")
    lines.append(f"- Tekilleştirme sonrası kayıt: `{metadata.get('deduped_count')}`")
    lines.append(f"- Silinen çift kayıt: `{metadata.get('removed_duplicate_count')}`")
    lines.append(f"- Nihai çıktı kayıt sayısı: `{metadata.get('final_output_count')}`")
    lines.append(f"- Out of scope çıkarılan kayıt: `{metadata.get('out_of_scope_removed')}`")

    for label in ["High", "Medium", "Low", "Out of scope"]:
        count = int((df["Thesis_Relevance_Label"] == label).sum()) if "Thesis_Relevance_Label" in df.columns else 0
    ...
    lines.append(f"- Çift kayıt grubu: `{len(duplicate_report)}`")
    ...
    if high.empty:
        ...
        lines.append("| Name | Year | Title | Score | Duplicate | Full text | Abstract source | Role | Suggested chapter | Keywords |")
        lines.append("|---|---:|---|---:|---|---|---|---|---|---|")

        for _, r in high.iterrows():
            lines.append(
                "| {name} | {year} | {title} | {score} | {dup} | {ft} | {asrc} | {role} | {chapter} | {kw} |".format(
                    ...
                    dup=safe_str(r.get("Duplicate_Status", "")),
    ...
    lines.append("")
    lines.append("## 3. Çift Kayıt Raporu")
    lines.append("")
    if duplicate_report.empty:
        lines.append("Çift kayıt bulunmadı.")
    else:
        lines.append("| Duplicate Key | Count | Kept Title |")
        lines.append("|---|---:|---|")
        for _, r in duplicate_report.head(50).iterrows():
            lines.append(
                f"| {safe_str(r.get('Duplicate_Key', '')).replace('|', '/')} | "
                f"{safe_str(r.get('Duplicate_Count', ''))} | "
                f"{safe_str(r.get('Kept_Title', '')).replace('|', '/')[:160]} |"
            )

    lines.append("")
    lines.append("## 4. Full Text'ten Çıkarılan Abstract Örnekleri")
    lines.append("")
    ...
    lines.append("")
    lines.append("## 5. Bulgular/Sonuç Çıkarımı Örnekleri")
    lines.append("")
...
st.caption(
    ...
    "PDF full text abstract çıkarımı, otomatik özet, bulgu/sonuç sütunları, "
    "tekilleştirme ve out-of-scope filtreleme ekler."
)
...
with st.sidebar:
    ...
    st.header("Filtreleme ve tekilleştirme")

    enable_dedup = st.checkbox(
        "Aynı makaleleri tekilleştir",
        value=True
    )

    include_out_of_scope = st.checkbox(
        "Out of scope kayıtları çıktıya dahil et",
        value=False
    )
...
if run:
    ...
    try:
        ...
        result, duplicate_report, metadata = analyze_dataframe(
            df=df,
            use_fulltext=use_fulltext,
            pdf_base_dir=pdf_base_dir,
            max_pdf_pages=max_pdf_pages,
        ...
        out_duplicates_csv = out_dir / "notion_literature_duplicate_report.csv"
        out_duplicates_xlsx = out_dir / "notion_literature_duplicate_report.xlsx"
        out_metadata_json = out_dir / "notion_literature_relevance_run_metadata.json"

        result.to_csv(out_csv, index=False)
        result.to_excel(out_xlsx, index=False)

        duplicate_report.to_csv(out_duplicates_csv, index=False)
        duplicate_report.to_excel(out_duplicates_xlsx, index=False)

        write_markdown_report(result, duplicate_report, metadata, out_md)
        ...
        c1.metric("Girdi kayıt", metadata["original_count"])
        c2.metric("Tekilleştirme sonrası", metadata["deduped_count"])
        c3.metric("Silinen çift kayıt", metadata["removed_duplicate_count"])
        c4.metric("Çıktı kayıt", metadata["final_output_count"])
        c5.metric("High", int((result["Thesis_Relevance_Label"] == "High").sum()))
        c6.metric("Medium", int((result["Thesis_Relevance_Label"] == "Medium").sum()))
        c7.metric("Full text kullanılan", int((result["Full_Text_Used"] == "Yes").sum()))

        if not include_out_of_scope:
            st.info(
                f"Out of scope kayıtlar çıktıya dahil edilmedi. "
                f"Çıkarılan out of scope kayıt sayısı: {metadata['out_of_scope_removed']}"
            )
        else:
            st.warning("Out of scope kayıtlar çıktıya dahil edildi.")
        ...
        st.code(
            ...
            f"{out_md}\n"
            f"{out_profile}\n"
            f"{out_duplicates_csv}\n"
            f"{out_duplicates_xlsx}\n"
            f"{out_metadata_json}",
            language="text"
        )

        if not duplicate_report.empty:
            st.subheader("Çift kayıt raporu")
            st.dataframe(duplicate_report, use_container_width=True, height=300)
        else:
            st.success("Çift kayıt bulunmadı veya tekilleştirme kapalı.")
